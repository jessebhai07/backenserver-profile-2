...
// Define Mongoose Schema for storing event images
const ImageSchema = new mongoose.Schema(
  {
    imageUrl: { type: String, required: true }, // The URL of the uploaded image (Cloudinary URL)
    date: { type: Date, required: true }, // The date associated with the event
  },
  { timestamps: true } // Automatically adds createdAt and updatedAt fields
);

const Image = mongoose.model("Image", ImageSchema); // Create a model from the schema

// Multer configuration for Cloudinary storage (Event Images)
const EventsStorage = new CloudinaryStorage({
  cloudinary, // Use already configured Cloudinary instance
  params: {
...
const ImageUpload = multer({ storage: EventsStorage }); // Set up multer to handle file uploads using Cloudinary storage

// API to upload image for events
app.post("/api/upload", ImageUpload.single("image"), async (req, res) => {
  try {
    const { date } = req.body; // Extract the event date from the request body

    // Check if the date is provided, return an error if not
    if (!date) {
      return res.status(400).json({ error: "Date is required" });
    }

    // Create a new image entry with the Cloudinary URL and event date
    const newImage = new Image({
      imageUrl: req.file.path, // URL of the image uploaded to Cloudinary
    ...
    // Send the saved image data as the response
...
app.get("/api/events", async (req, res) => {
  try {
    ...
    res.json(images); // Return the images as the response
  } catch (error) {
    res.status(500).json({ error: "Error fetching images" }); // Handle errors
  }
});
